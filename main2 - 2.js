          ...
          console.log(removed_data_id);
            ...
            // console.log("after" + data.length + " " + removed_data.length);
             ...
             tooltip.transition()
                .duration(500)
                .style("opacity", 0);
            // refreshShapes();  - do not refreshShape - removeDot will reappear
            // tooltip.remove();
...
function removeData(thisObject, data_id) {
  ...
  refreshShapes(); 
  // console.log("remove thisObject "+ thisObject); //[object Window]
  // console.log("remove thisObject[0] "+ thisObject[0]); //[object Window]
  // console.log("remove id"+ thisId);
  // removed_data_id.push(thisId);
  // lastRemove.push(thisObject); // need to set "active" on undo
  // dataforCal = dataforCal.filter(function(e) { return e.id !== thisId});
  // console.log("(in remove) dataforCal.length "+ dataforCal.length)


 // console.log("Clicked H this.id "+ this.id + "; d.id " + d.id);  //this.id is diagnostic, d.id is id in data
