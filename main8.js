...
// Purpose: read data from csv, create interactive scatter plot that allow the users to evualute
// 1) their outlinerness/ and sensitivity to the "model"
// 2) change distance protocol for dispatching helicopter 
...
d3.csv("data/HTData8.csv", function(error, data) {
...
// **** Add Tooltip & legend area to webpage ******
...
function refreshShapes() {  //BEGIN function refreshShape
  ...
  //    3c) on click remove "this" data  
    // bugout.log("In refreshShape");
    // bugout.log(baseData[0]);  // basedata is passed properly
  ...
  // filter data with checkbox choice
	...
	d3.selectAll('input[type=checkbox]').each(function(){
		...
		if (this.checked){
			bugout.log(this.value + " checked by user");
      legendData.push(this.value);
		}
		else {
			bugout.log(this.value + " unchecked by user");
  ...
  d3.selectAll('input[type=radio]').each(function(){
    // bugout.log("radio");
    bugout.log(this.value, this.checked, "radio button");
 });

  // bugout.log(checks);  
	// bugout.log("T "+checks["T"]);
	// bugout.log("H "+checks["H"]);
  ...
  // bugout.log(data[1]);
  // bugout.log(data[1].mahaDis);

	// bugout.log("data - # " + data.length);
	// bugout.log("baseDataL - # " + baseData.length);
	...
	// bugout.log([d3.min(data, xVals)-1, (d3.max(data, xVals)+1)/zoomfactor]); //[11,133]
  ...
  var mode_type = d3.set(data.map(function(d) { return d.mode; }));
    ...
    function calculateRegression (my_dataforCal){
        ...
        // bugout.log("called calculateRegression" )
        // bugout.log("my_dataforCal.length "+ my_dataforCal.length )
        var m = 1;
        var b = 0; 
//                bugout.log("calculate regression" + my_dataforCal.length);
...
//            var yHats = function(d){return xVals * m + b;};  //not used
  ...
  var colorscaleSet = d3.scale.category20();
  ...
  //"Outcome"
    ...
    var cValueO = function(d) { return d["outcome_28day"];},   // d3 does color magics
       colorO = colorscaleSet;

    var radius = 3;

  // bugout.log(checks["T"] , checks["H"]);

	if (checks["T"] == true){ //BEGIN   draw dots (circles) - T
		        //svg.selectAll('path.line')
		// bugout.log("T dot = true " + TdataforCal.length);
		addCircles_for_T(TdataforCal);
        // END   draw dots - T 
	}
	if (checks["H"] == true){  //BEGIN   draw dots - H
		// bugout.log("H dots== true");
...
function addSquare_for_H(myData) {
    ...
    // bugout.log("addSquare_for_H ", radius, cValueD, myData.length);
    ...
    .style("fill", function(d){
      ...
      } else if (colorscale == "outcome") {
          return colorscaleSet(d.outcome_28day);
      } else {
    ...
    squares.on("click",function(d) {  // onclick H
      ...
      // then call refreshShapes()
          // bugout.log("Clicked H this.id "+ this); //[object SVGRectElement]
          // bugout.log("Clicked H this.pid "+ this.pid);
          bugout.log("User Clicked H dot: this.id(diagnostic) "+ this.id + " " + d.outcome_28day+ " " +"; d.pid " + d.pid + "Maha dis "+ d.mahaDis);  //this.id is diagnostic, d.id is id in data
          var dot_id = this.id;
          var data_id = d.pid;
          // bugout.log("before filter" + baseData .length + "data_id" + data_id);
          // bugout.log(baseData[0]);
          ...
          // bugout.log("after filter" + baseData.length);
          bugout.log("removed_data_id:" +removed_data_id);
...
function addCircles_for_T(myData) {
  // bugout.log("addCircles_for_T ", radius, cValueD, myData.length);
...
circles.on("mouseover", function(d) {   //BEGIN mouseover on T
 ...
 circles.on("click",function(d) {  //remove on click  (T set)
      ...
      // save "this" to lastRemove
      // then call refreshShapes()
      bugout.log("User Clicked T dot: this.id(diagnostic) "+ this.id + " " + d.outcome_28day+ " " +"; d.pid " + d.pid + "Maha dis "+ d.mahaDis);  //this.id is diagnostic, d.id is id in data
      var dot_id = this.id;
      var data_id = d.pid;
      bugout.log("before filter" + baseData .length + "data_id" + data_id);
      bugout.log(baseData[0]);
...
function removeData(thisObject, data_id) {
  // bugout.log("Inside removeData");
  // bugout.log("baseData " +baseData.length);  // basedata is passed properly
  // // bugout.log(data[0]);  // *** data is NOT passed properly***
  // bugout.log("dataforCal " +dataforCal.length);  // dataforCal is passed properly
  // bugout.log("TdataforCal " + TdataforCal.length);  // TdataforCal is passed properly
  // bugout.log("HdataforCal " + HdataforCal.length);  // TdataforCal is passed properly
  ...
  bugout.log("doing removed_data");
  // bugout.log(thisRemovedData);  // e.g [Array(1), Array(1)]  ****data in the first element***
  // bugout.log(thisRemovedData[0].x);

  removed_data_id.push(data_id);
  lastRemove.push(thisObject); // need to set "active" on undo

  thisRemovedData = thisRemovedData[0];
  bugout.log(thisRemovedData);  //e.g. Object {distance_km: 85, out_of_hospital_time_minute: 176, mode: "H", mode_long: "Helicopter", diagnostic_smur: "strokes"…}
  // bugout.log(thisRemovedData.x, thisRemovedData.y,  thisRemovedData.pid);
...
function undoLast() {
  if (removed_data.length >0 ) {
    bugout.log("!!!Inside undoLast");
    // bugout.log("baseData " +baseData.length);  // basedata is passed properly
    bugout.log(removed_data_id); //["14"]
    //bugout.log(lastRemove); // e.g [rect#strokes] or eg. 2 [rect#severe trauma, rect#acute coronary syndrome]
    var DotTobeAdded = lastRemove.pop();  //not really used for redraw, used dataTobeAdded instead
    //bugout.log(DotTobeAdded);  //e.g <rect class="dots" id="strokes" width="9" height="9" x="309.344262295082" y="207.99999999999994" style="opacity: 1;" fill="#FFFFFF"></rect>

    var dataTobeAdded = removed_data.pop();
    bugout.log("dataTobeAdded back");    
    bugout.log(dataTobeAdded); 

    baseData = baseData.concat(dataTobeAdded);
    refreshShapes(); 
  }
  else {
    alert("Sorry! No stored last removed datapoint");
  }
